...
void initIR(void){
    ...
    AD1CON2bits.CSCNA = 1; //Scan inputs selected by AD1CSSL reg
...
stateType checkSensorValues(void){
    ...
    unsigned int leftRead, rightRead, midRead;
    ...
    if (rightRead > MID_VOLTAGE_VAL){
        rightIR = LINE;
    }
    else rightIR = FLOOR;

    if (midRead > MID_VOLTAGE_VAL){
        midIR = LINE;
    }
    else midIR = FLOOR;
